...
## 2026-06-25 - [Optimize ToolAdapter with memoization]
**Learning:** `ToolAdapter.get_enabled_tools` was recreating nested dictionary schemas for every call, adding overhead to engine initialization and agent switching. Also identified that iterating over the user-provided `enabled_utils` list is unsafe as it can lead to duplicates and non-canonical ordering.
**Action:** Implemented a private class-level cache `_formatted_cache` with a helper method. Refactored `get_enabled_tools` to iterate over the canonical `TOOL_DEFINITIONS` while checking against a set of `enabled_utils`, and returning `.copy()` of cached items. Measured a ~2.8x speedup in tool registration.

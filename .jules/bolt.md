...
## 2026-06-25 - [Optimize tokenization with encode_ordinary]
**Learning:** `tiktoken.encode_ordinary()` is faster than the standard `encode()` because it bypasses special token regex checks. This is safe for standard chat messages and provides a measurable speedup in token-counting-heavy loops.
**Action:** Replaced `encode()` with `encode_ordinary()` in `Message.get_token_count` for both content and tool calls.

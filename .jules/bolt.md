...
## 2026-06-25 - [Optimize tokenization and cache management]
**Learning:** `tiktoken.encode_ordinary()` is ~10-25% faster than `encode()` for standard chat content as it bypasses special token parsing. Also, when using multiple incremental caches (OpenAI format, no-system, dumped JSON), a centralized `_reset_state` method that rebuilds all caches in a single pass is critical for both consistency and maintaining O(1) read performance across the API.
**Action:** Replaced `encode()` with `encode_ordinary()` in `Message.get_token_count` and refactored `ConversationManager._reset_state` to rebuild all internal caches in a single pass.

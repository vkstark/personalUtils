...
**Learning:** `get_summary()` was an O(N) operation because it recalculated role counts by iterating over the entire message history on every call. For a 4,000-message conversation, this took ~0.76ms.
**Action:** Implemented incremental role tracking using a `defaultdict(int)` in `ConversationManager`. Added a `_reset_state()` helper to centralize state recalculation after bulk operations (trimming, clearing, loading). Reduced `get_summary()` latency to ~0.0046ms (~165x speedup).

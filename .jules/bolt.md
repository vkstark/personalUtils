...
**Learning:** Found that sequential `add_message` calls (e.g., in tool execution loops or user-assistant turns) caused O(N^2) write amplification because the entire history was re-serialized and written to disk on every call.

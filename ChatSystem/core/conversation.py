...
When users ask you to perform tasks, analyze if any tools can help. Break complex tasks into steps and execute them systematically. Always explain what you're doing and provide clear results."""
    ...
    def get_messages(self, include_system: bool = True) -> List[Dict[str, Any]]:
        ...
        # Optimize by filtering the existing cache instead of re-serializing
    ...
    def _save_history(self):
        ...
        try:
            ...
            # Convert messages to dict format using cached dumps
            history_data = {
                "model": self.model,
    ...
    def _load_history(self):
        ...
        try:
            ...
            self._total_tokens = self.count_tokens(self.messages)
            # Initialize caches to enable O(1) appends later
            self._cached_openai_messages = [msg.to_openai_format() for msg in self.messages]
            self._cached_dumped_messages = [msg.model_dump() for msg in self.messages]

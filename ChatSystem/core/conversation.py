...
When users ask you to perform tasks, analyze if any tools can help. Break complex tasks into steps and execute them systematically. Always explain what you're doing and provide clear results."""
    ...
    def batch_saves(self):
        """
        A context manager to batch multiple history writes into a single operation.

        When inside this context, calls to `add_message` or other methods that
        trigger `_save_history` will not write to disk immediately. Instead,
        the history will be saved once when the outermost context block exits,
        provided that at least one save was requested.
    ...
    def _save_history(self):
        ...
        warnings.
        """
        # If we are batching, just mark as needing save and return
        ...
        try:
            ...
            # Reset needs_save flag after successful write
            self._needs_save = False

...
When users ask you to perform tasks, analyze if any tools can help. Break complex tasks into steps and execute them systematically. Always explain what you're doing and provide clear results."""
    ...
    def _reset_state(self):
        """
        Resets and re-calculates the internal state based on the current messages.
        This includes total tokens and role counts.
        """
        self._total_tokens = 0
        self._role_counts = defaultdict(int)
    ...
    def _load_history(self):
        ...
        try:
            ...
            # Bolt: Update state using centralized method for accuracy
            self._reset_state()
    ...
    def get_summary(self) -> Dict[str, Any]:
        ...
        # Bolt: Optimize get_summary to O(1) via incremental role tracking
        # We filter out roles with 0 counts to keep the summary clean
        role_counts = {role: count for role, count in self._role_counts.items() if count > 0}

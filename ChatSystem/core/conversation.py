...
When users ask you to perform tasks, analyze if any tools can help. Break complex tasks into steps and execute them systematically. Always explain what you're doing and provide clear results."""
    ...
    def _invalidate_cache(self):
        """Invalidates the cached messages."""
    ...
    def get_messages(self, include_system: bool = True) -> List[Dict[str, Any]]:
        ...
        # If including system prompt, return full list
        if include_system:
            # Return a shallow copy of the list to prevent external modification
            return self._cached_openai_messages[:]

        # If not including system prompt, filter the cached list (faster than re-serialization)
        return [
    ...
    def _save_history(self):
        ...
        try:
            ...
            # Build dumped cache if it doesn't exist
            ...
            with open(self.history_file, "w", encoding="utf-8") as f:
                # Optimized serialization: remove indentation and use pre-dumped JSON-ready dicts
